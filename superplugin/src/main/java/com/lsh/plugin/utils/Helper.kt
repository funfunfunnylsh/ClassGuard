package com.lsh.plugin.utils

import com.lsh.plugin.entension.findWord
import java.io.File
...
private val importPattern = Pattern.compile("\\s*import\\s+(.*)\\.R\\s")
...
fun File.insertImportXxxIfAbsent(newPackage: String) {
    ...
    //如果使用了R类但没有导入，则需要导入
    val isImported = importPattern.matcher(text).find()
    val needImportR = text.findWord("R.") != -1 && text.findWord(importR) == -1 && !isImported
    //如果使用了BR类但没有导入，则需要导入
